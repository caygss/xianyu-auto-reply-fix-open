...
Prepare this modified Xianyu management project for fee-based distribution as an open-source Windows installation package. The package may be sold on Xianyu as an installation package or installation service, while recipients retain the right to inspect, modify, and redistribute the project under the original AGPL-3.0 terms. The delivered Windows package will contain compiled application artifacts rather than Python source files.
...
- Build a Windows executable distribution from the exact source version instead of bundling Python source files inside the archive.
- Bundle the static web assets, configuration template, and Playwright Chromium runtime required by the compiled application.
...
The installation archive will contain the compiled Windows application, bundled static assets and browser runtime, Windows installation/launch scripts, documentation, license notices, and a `SOURCE-CODE.md` pointer. It will not contain Python source files. `SOURCE-CODE.md` will contain the public repository URL, exact version tag, exact source commit, and a note that the corresponding source is available at no charge.
...
6. Compile the Windows application and bundle its required static assets and Chromium runtime.
7. Re-run tests, build the Windows archive, and verify that the archive contains no Python source or runtime state and points to the exact published source.
...
- The package starts the compiled application without requiring Python or Docker.
- The package contains no application `.py` source files while retaining the corresponding source in the public repository.
