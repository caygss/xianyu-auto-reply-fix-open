...
- `verification_required`: set when the sender started but did not prove successful completion. Such rows require human verification and cannot be automatically resent, marked sent, platform-confirmed, or finalized.
...
`DeliveryOrchestrationService.begin_send(request, claim_token)` is the only configured-send preflight API. It normalizes the request, verifies the exact persisted claim payload (`token`, `quantity`, `mode`, `idempotency_key`, `item_id`, and existing order/card/account/line scope), and atomically writes `send_started_at` before returning success.

`mark_verification_required(request, claim_token)` durably marks an already-started send as unknown without transitioning it to `sent` or `failed`.
...
5. If heartbeat fails while sender is unfinished, mark orchestration `verification_required`, cancel/collect the sender, and do not create a `sent` finalization anchor.
